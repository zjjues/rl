...
class ObjectiveSemanticAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        preference_relevance_gate: Optional[PreferenceRelevanceGate] = None,
    ) -> None:
        self.model = model
        ...
        self.preference_relevance_gate = preference_relevance_gate
        ...
        if self.preference_relevance_gate is not None:
            if self.preference_relevance_gate.encoder_model != self.model_name:
                raise ValueError("preference relevance gate encoder model mismatch")
            if self.preference_relevance_gate.encoder_revision != self.model_revision:
                raise ValueError("preference relevance gate encoder revision mismatch")
            if self.preference_relevance_gate.embedding_dim != self.coefficients.shape[0]:
                raise ValueError("preference relevance gate embedding dimension mismatch")
    ...
    def fit(
        ...
        *,
        ...
        nli_model_revision: str = "",
        nli_batch_size: int = 32,
        preference_relevance_gate_path: str = "",
        ...
        preference_relevance_gate = (
            PreferenceRelevanceGate.load(preference_relevance_gate_path)
            if preference_relevance_gate_path else None
        )
        return cls(
            model,
            coefficients,
            ...
            preference_relevance_gate=preference_relevance_gate,
        )
    ...
    def _apply_preference_relevance_gate(
        self,
        descriptions: Sequence[str],
        profiles: np.ndarray,
        embeddings: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        if self.preference_relevance_gate is None:
            return np.asarray(profiles, dtype=np.float32)
        if embeddings is None:
            embeddings = self._embed([str(text) for text in descriptions])
        return self.preference_relevance_gate.apply(embeddings, profiles)
    ...
    def predict_profiles(self, entries: Sequence[Tuple[str, str]]) -> np.ndarray:
        if self.profile_decoder == "polarity_prototype":
            ...
            return self._apply_preference_relevance_gate(
                [str(description) for _, description in entries],
                profiles,
                query_embeddings,
            )
            ...
            if missing:
                ...
                if self.profile_decoder in {
                    ...
                    if self.profile_decoder == "nli_prototype_gated":
                        ...
                        relevance_embeddings = query_embeddings
                        class_similarities = (
                ...
                profiles = self._apply_preference_relevance_gate(
                    missing, profiles, relevance_embeddings
                )
        ...
        profiles = self.predict_profiles_from_embeddings(embeddings)
        return self._apply_preference_relevance_gate(
            descriptions, profiles, embeddings
        )
    ...
    def metadata(self, train_labels: Sequence[str]) -> Dict[str, object]:
        return {
            ...
            "nli_relevance_gate": (
                (
                    ...
                    if self.profile_decoder == "nli_relevance_gated"
                        ...
                        f"nearest centroid among {len(OBJECTIVE_KEYS)} independently "
                        "templated preference classes and a neutral class"
                        if self.profile_decoder == "nli_prototype_gated"
            ...
            "polarity_prototype_rule": (
                f"nearest centroid among {len(OBJECTIVE_KEYS)} low, "
                f"{len(OBJECTIVE_KEYS)} high, and one neutral class"
            ...
            "preference_relevance_gate": (
                {
                    ...
                    "threshold": float(self.preference_relevance_gate.threshold),
                    "encoder_model": self.preference_relevance_gate.encoder_model,
                    "encoder_revision": self.preference_relevance_gate.encoder_revision,
                    "metadata": dict(self.preference_relevance_gate.metadata),
                }
                if self.preference_relevance_gate is not None
                else {"enabled": False}
            ),
