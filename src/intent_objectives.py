...
PREFERENCE_OBJECTIVE_KEYS = (
    "distance", "energy", "safety", "task", "time", "threat"
)
SAFETY_CONSTRAINT_KEYS = ("collision",)
REWARD_OBJECTIVE_KEYS = (
    "distance", "energy", "collision", "safety", "task", "time", "threat"
)
# Backward-compatible name for semantic/profile decoders. Collision is excluded:
# language may change safety margin preference but never the contact penalty.
OBJECTIVE_KEYS = PREFERENCE_OBJECTIVE_KEYS
DEFAULT_INTENT_REWARD_PROFILE: Dict[str, float] = {
    key: 1.0 for key in REWARD_OBJECTIVE_KEYS
}


def _profile(**overrides: float) -> Dict[str, float]:
    unknown = sorted(set(overrides) - set(PREFERENCE_OBJECTIVE_KEYS))
    if unknown:
...
UAV_INTENT_REWARD_PROFILES: Dict[str, Dict[str, float]] = {
    "safety_first": _profile(safety=1.6, task=0.8, time=0.7, threat=1.4),
    ...
    "aggressive_pursuit": _profile(distance=1.5, safety=0.6, task=1.3, time=1.5),
    "cautious_exploration": _profile(distance=0.8, safety=1.5, time=0.6),
    "load_balancing": _profile(task=1.2, safety=1.1),
    "formation_keeping": _profile(safety=1.3, distance=0.9),
    ...
    "center_convergence": _profile(distance=1.4, task=1.2, time=1.3),
    ...
    "full_coordination": _profile(safety=1.2, task=1.2),
    "altitude_separation": _profile(safety=1.4, task=0.9),
    "speed_modulation": _profile(energy=1.2, time=1.1),
    "reactive_avoidance": _profile(safety=0.8, task=1.1, time=1.2),
    "predictive_planning": _profile(safety=1.4, task=1.1, time=0.9),
