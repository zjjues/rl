# Result Card: uav_semantic_cbf_smoke_v1

- Evidence level: `smoke`
- Seeds: `7`
- Evaluation episodes per seed/tier: `3`
- Primary objective: Verify intent-conditioned pairwise CBF action projection, residual compatibility, and safety-margin controllability.

## Variants

- `prototype_residual_cbf`: representation=`objective_grounded_semantic`, algorithm=`imappo`
- `prototype_prior_cbf`: representation=`objective_grounded_semantic`, algorithm=`imappo`
- `prototype_prior_no_filter`: representation=`objective_grounded_semantic`, algorithm=`imappo`
- `nonsemantic_residual_cbf`: representation=`none`, algorithm=`mappo`
