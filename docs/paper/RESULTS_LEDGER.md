...
## 2026-08-24 正式消融 partial v2（曾通过 1-run 审计，现 superseded）
...
- 当时的 1-run valid audit 保存在 Git commit `fd03809`；当前工作树只保留随后覆盖它的两 seed invalid 审计，避免把历史中间状态误当活动证据。
- 随后的 seed 11 `--resume` 暴露单目标 `objective` 被冗余扩展为 `objectives=[...]`，导致完整 spec hash 漂移。虽然 seed 7/11 implementation hash 相同且 seed 11 wall/CPU=1471.20/1314.27 s、hard collision/task/return=0.0044/0.608608/−19.178564，strict validator 判 manifest/result protocol mismatch。
- 因 runner 的幂等修复会改变 implementation hash，整个两 seed fragment 已移至 `uav_imappo_ablation_paper_v2_superseded_resume_objective_drift_20260824`，活动正式进度重置为 0/100。审计：`docs/paper/audits/uav_imappo_ablation_resume_objective_drift_superseded_audit.json`。
...
- 全量测试：**183 passed, 14 warnings**。该结果属于复现基础设施验证，不是算法效果结果。
